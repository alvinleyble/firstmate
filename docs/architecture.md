...
The existing turn-end guard remains the final backstop for all harness-engine protocols, with pi-signed sharing Pi's protocol and the `--claude` mode cooperating with the auto-arm claim.
...
Codex, standalone Kimi, and agy classify unknown behind explicit probes until a semantic source is live-verified for them, and Grok keeps one clearly isolated rendered-tail fallback that can only ever classify a Grok task.
...
That keeps spawn launch compatible across claude, codex, opencode, pi, pi-signed, grok, kimi, agy, and muse while preserving the requested profile for later audit.
...
After a ship teardown clears those proofs, the same script runs that project's post-landing cleanup: it reports whether the project keeps a `docs/build/project-build-progress.md` tracker, returns the primary clone to the default branch and prunes its stale `origin` tracking refs, pushes pending Supabase database migrations and deploys Supabase Edge Functions touched by the last few commits when the project carries a `supabase/` directory, then sweeps the project repository for prunable worktree registrations and landed branches.
That sweep deletes a local branch only when it is not the default branch, is checked out in no worktree, backs no live task recorded in `state/`, is provably landed, and is task-associated - named with the `fm/` task-branch prefix, or carrying an unambiguous pull request of its own merged into the default branch.
Landing is proven by reachability from the default branch first, then by the squash-safe content and merged-PR proofs the teardown header owns, because a squash-landed branch is never an ancestor of the default branch and an ancestry-only test would leave it stuck forever; a landed branch that is not task-associated is reported for the owner to delete by hand, and a branch whose landing cannot be proven is always kept.
It deletes a remote branch only through that same merged-PR proof, never from the branch name alone, and keeps and reports a branch whose merged pull request targeted any base other than the default branch.
