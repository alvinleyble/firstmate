...
repair_line() {
  ...
  case "$HARNESS" in
    ...
    grok)
      ...
      ;;
    agy)
      printf '%s%s\n' "$prefix" 'watcher supervision needs Stop-hook-owned automatic recovery; confirm bin/fm-agy-hooks-install.sh status reports the hook installed before ending the turn.'
      ;;
    *)
...
ordinary_wake_line() {
  case "$HARNESS" in
    ...
    grok)
      ...
      ;;
    agy)
      printf '%s\n' '- Ordinary wake: the Stop hook (bin/fm-agy-stop-hook.sh) already owns watcher continuity; drain and handle the wake, and do not arm another cycle yourself.'
      ;;
    *)
